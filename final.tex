...
The research report, Mathematics and Science Teacher Shortage: Fact and Myth by Ingersoll, Perda of University of Pennsylvania, reexamines the issue of mathematics and science teachers to evaluate whether or not there is a supply-side deficit of staff, and if so, to what extent. The causes of school staffing problems can be realized by two perspectives to illustrate its consequences. The first presents itself as a causal progression from increased student enrollment, deficiency in teachers, and institution staffing issues which inevitably leads to decreased student performance. The second, also a causal progression, examines the issue from an organizational perspective, which if its issues of teacher turnover and staffing problems are not properly addressed, will also cause a decrease in student performance. It is important to note that there is a distinction between these two perspectives—one which is dependent on enrollment, while the other is dependent on the organizational structure of the institution. Relevancy can be drawn from both perspectives when applied to our problem. For example, the initial perspective presented suggests that the more students who enroll in the CSE 14X series, the more TA’s need to be hired, or else we may be faced with a supply-deficit of TA’s which cannot properly accommodate the volume of students. Another approach is to view our problem from the organizational perspective: how can we optimize TA scheduling for better student-TA experience?

Our model assumes that seniority is measured by the amount of quarters an individual has taught. It is important to note that our metric for seniority cannot truly capture the experience level of an individual, even though correlations can be drawn. These subtleties are realized in the paper, Perception of Adequacy by Schmalenberg, Kramer. The paper entails that the adequacy of staffing is dependent on the [TA’s] perceptions of the work environment in the unit. There are three main points to consider: competency of the staff, teamwork, and a flexible delivery system. Applying this to our problem, we see that two of the three considerations are met—seniority of staff, as well as a dynamic delivery system for students to ask questions. If we choose to take teamwork into consideration, pairing two TA’s to answer one request could improve delay times—as one may be more akin to spotting issues quicker than another. Complexity rises when these constraints are implemented into our current model, but these are remarks which require further reexamining in the future when more data sets are accessible.
...
\begin{figure*}[ht]
   ...
   \begin{tikzpicture}
      ...
      \end{axis}
   \end{tikzpicture}
   \caption{\small{Simulation results for the same day as in Figure ?. Response times are averages from 100 simulation runs, each of which was run at 1,080x real time. Differences between observed and simulation wait times are likely the result of oversimplifications in our simulated dequeue and service policies.}}
\end{figure*}
