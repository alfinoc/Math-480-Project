...
In order to compare our solution with the scheduling approach currently in use, we first needed to simulate the system described above in \textsc{Background}. The script in \texttt{existing.py} works in three phases. First, each senior TA staffing quota is satisfied by the senior TA with the highest preference for that slot. Second, all TAs are assigned to slots in order from most to least senior, with TAs choosing their most preferred slot of those currently available. We end this stage when all minimum quotas are met and each TA has been assigned two slots. And finally we assign surplus hours (those that TAs would like to work \textit{in addition} to the minimum two previously assigned, just as in previous stages. At each stage, we make sure not to violate staffing requirement contraints.

This greedy algorithm could reach an unsolvable state where no TA is available to fill some minimum slot. Because TAs typically have only a few hours where they absolutely cannot work, the above approach outputs a feasible solution when provided the same input parameters as in \texttt{lp.sage}. (TODO: include actual existing.py results)

We evaluated schedules in terms of several quality metrics. We write \textit{total adherence} and \textit{total weighted adherence} for $z$ values as computed in equation (?) at $w = 0$ and $w = 1$ respectively. In addition to these aggregates, we look at the mean solution $x_{ij}a_{ij}$ and the mean solution $x_{ij}a_{ij}s{i}$, measures of average preference satisfaction per slot for regular and senior TAs respectively. Functions to compute these values are found in \texttt{evaluation.py}.
...
0.0 & 113.181 & 271.244 & 113.181 & 0.79 & 0.794 \\
   \end{tabular}
   \\[10pt]
   \caption{\small{Evaluation results for 10 generated schedules for various values of $w$ in Equation ?. Total and weighted adherence are z-values of Equation ? at $w = 0$ and $w = 1$ respectively. Mean preferences scores measure the average adherence to preference in assigned slots and are computed as mean $x_{ij}a_{ij}$ and mean $x_{ij}a_{ij}s_i$ respectively.}}
\end{table*}
