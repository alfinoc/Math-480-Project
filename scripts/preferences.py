...
# Inserts zeroes for a uniformly random number of courses and sections drawn from
# constants defined above. Only the first 5 hours of the day can be blacked out.
...
# Returns a gamma distribution centered on 'peak' as a vector of floats in [e, 1]
# where e is closer to zero (but never zero) with higher samples. The returned
# vector has give size, and peak must be a valid index in this vector.
...
# Converts an hour list representation to a day-keyed preference map. Assumes that
# the given list was generated by serialize().
